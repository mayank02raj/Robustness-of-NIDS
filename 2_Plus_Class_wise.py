...
# Calculate class-wise accuracy and F1-score
class_metrics = defaultdict(dict)

for label_idx, label_name in enumerate(label_encoder.classes_):
    true_positive = np.sum((np.array(test_preds) == label_idx) & (np.array(test_targets) == label_idx))
    false_positive = np.sum((np.array(test_preds) == label_idx) & (np.array(test_targets) != label_idx))
    false_negative = np.sum((np.array(test_preds) != label_idx) & (np.array(test_targets) == label_idx))
    true_negative = np.sum((np.array(test_preds) != label_idx) & (np.array(test_targets) != label_idx))

    class_accuracy = (true_positive + true_negative) / len(test_targets)
    class_f1_score = f1_score(test_targets, test_preds, labels=[label_idx], average='weighted')

    class_metrics[label_name]['Accuracy'] = class_accuracy
    class_metrics[label_name]['F1-Score'] = class_f1_score
